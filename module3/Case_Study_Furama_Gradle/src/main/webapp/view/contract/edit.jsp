...
<div class="container d-flex ">
    <form action="/contracts?action=edit" style="width: 70%;margin-left: 155px" method="post">
        <div class="form-group row">
            <label for="inputId" class="col-sm-3 col-form-label">Contract Id</label>
            <div class="col-sm-6">
                <input type="text" class="form-control" id="inputId" name="contractId" readonly value="${contract.id}">
            </div>
        </div>
        <div class="form-group row">
            <label for="inputName" class="col-sm-3 col-form-label">Contract Start Date</label>
            <div class="col-sm-6">
                <input type="text" class="form-control" id="inputName" name="startDate" value="${contract.startDate}">
            </div>
        </div>
        <div class="form-group row">
            <label for="inputDate" class="col-sm-3 col-form-label">Contract End Date</label>
            <div class="col-sm-6">
                <input type="text" class="form-control" id="inputDate" name="endDate" value="${contract.endDate}">
            </div>
        </div>
        <div class="form-group row">
            <label for="inputIdCard" class="col-sm-3 col-form-label">Deposit</label>
            <div class="col-sm-6">
                <input type="number" class="form-control" id="inputIdCard" name="deposit" value="${contract.deposit}">
            </div>
        </div>
        <div class="form-group row">
            ...
            <div class="col-sm-6">
                <select name="employee" id="inputType1">
                    <c:forEach items="${employees}" var="employee">
                        <option value="${employee.id}" ${contract.employeeId==employee.id?"selected":""}>${employee.name}</option>
                    </c:forEach>
                </select>
            </div>
        </div>
            ...
            <div class="col-sm-6">
                <select name="service" id="inputType2">
                    <c:forEach items="${services}" var="service">
                        <option value="${service.id}" ${contract.serviceId==service.id?"selected":""}>${service.name}</option>
                    </c:forEach>
                </select>
            </div>
        </div>
        <div class="d-flex ">
            <a href="/customerUsingServices" class="btn btn-secondary" style="margin-left: 200px">Back to list</a>
